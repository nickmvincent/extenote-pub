---
type: paidf_chapter
title: Flywheel design space
slug: 01e_options_for_flywheel
visibility: public
---
Status: needs tightening - repetitive and list-heavy; add concrete examples or diagrams.
...
In @why-collect-data we talked about the numerous combinations of sensors, forms, task settings, social structure from institutions, communities, etc. that might exist, and in the Appendix we discuss a number of formats and types of data for LLMs in particular.
...
Beyond the architectural patterns described above, there are several emerging approaches that could fundamentally reshape how public AI data flywheels operate:

**Data Cooperatives and Intermediaries**: In many cases, users may want to have data governed by community organizations (e.g., organized by domain/region/language) that hold rights and decide release cadence, licensing defaults, and benefit policies.
...
### Interactions between flywheel architectures

Different flywheel architectures can complement each other in practice:

- **Layered approaches**: A "PrivateCo" style web app might offer a one-click export to a public Git repository, combining low-friction data creation with transparent public storage.
- **Channel migration**: Users might start with passive telemetry-based contribution and later "graduate" to more active, peer-production style contributions as they become more invested.
- **Cross-instance federation**: Multiple community-run flywheel instances (each with their own governance) could periodically merge compatible datasets, creating network effects while preserving local autonomy.
- **Complementary friction levels**: High-friction peer production creates gold-standard, high-quality contributions, while low-friction mechanisms provide scale and coverage. Both can feed into the same downstream training pipelines.

The key insight is that these approaches need not be mutually exclusive—a well-designed ecosystem might support multiple on-ramps for different user preferences and contribution styles.

### Transitioning from opt-in flywheel to federated learning
